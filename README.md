...
项目内置一个本地数据看板，用于查看自动化运行过程中的制造、兑换、购买和收取记录。看板只读取本机数据，不会上传到远端。
...
当前看板主要展示：
- 总事件数、总数量、购买成本、兑换成本、预期利润等核心指标。
- 按事件类型聚合的统计，例如制造、购买、兑换、收取。
- 按物品或部门聚合的成本、预期收益、预期利润。
- 最近事件明细，便于回看每次自动化执行读到的成本、税后价、产量和收益。


说明：
- `analytics_summary` 适合在命令行快速查看统计摘要。
- `analytics_server` 适合用浏览器长期观察数据。
- `logs/analytics.sqlite3` 属于本地运行数据，默认不会提交到 Git。
